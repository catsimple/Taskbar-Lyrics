...
void 任务栏窗口类::剩余宽度检测()
{
    auto 线程函数 = [&] () {
        while (true)
        {
            ...
            RECT 任务栏_矩形, 开始按钮_矩形, 活动区域_矩形, 通知区域_矩形;
...
void 任务栏窗口类::监听注册表()
{
    ...
    auto 获取注册表值 = [this, 读取注册表] () {
...

        {
            ...
            if (!读取注册表(路径, 键, 值))
            {
                bool 新值 = static_cast<bool>(值);
                if (this->呈现窗口->深浅模式 != 新值)
                {
                    this->呈现窗口->深浅模式 = 新值;
                    变化 = true;
                }
    ...
    auto 线程函数 = [this, 获取注册表值] () {
        while (true)
        {
            if (!this->注册表句柄)
            {
                ...
                if (RegOpenKeyEx(HKEY_CURRENT_USER, path.c_str(), NULL, KEY_NOTIFY, &this->注册表句柄) != ERROR_SUCCESS)
                {
                    this->注册表句柄 = nullptr;
            ...
            else if (result == ERROR_KEY_DELETED || result == ERROR_HANDLE_REVOKED)
            {
                if (this->注册表句柄)
                {
                    RegCloseKey(this->注册表句柄);
                    this->注册表句柄 = nullptr;
                }
            }
            else
